...
func (n *NoteRepository) Save(note *domain.Note) (*domain.Note, error) {
	res, err := n.db.Exec(`INSERT INTO notes (profile, name, content, created_at, last_modified)
		VALUES (?, ?, ?, ?, ?);`, note.Profile, note.Name, note.Content, note.CreatedAt, note.LastModified)
	if err != nil {
		return nil, err
	}
...
//
// Regardless of any value provided, the CreatedAt field will be ignored for this operation.
...
func (n *NoteRepository) FindByProfileAndID(profile string, id int) (*domain.Note, error) {
	res := n.db.QueryRow("SELECT * FROM notes WHERE id = ? AND profile = ? AND active = 1", id, profile)
	note, err := deserializeNote(res)
	if err != nil && !errors.Is(err, sql.ErrNoRows) {
		return nil, err
	}
